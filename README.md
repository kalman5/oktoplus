...
#### TODO
  ...
  - **SDS-style strings (Phase 1+2: done)**: `okts::stor::string` (16 B SSO-or-heap, vs std::string's 32 B in libstdc++) replaces `std::string` everywhere in the storage path -- list/deque/vector slots AND the per-shard hash-map keys (Phase 2). Halves cache footprint per element on iteration AND per slot in the absl::flat_hash_map bucket array, and lets jemalloc serve heap blocks from a smaller size class because the allocation is exactly `size` bytes (no NUL terminator, no capacity slack).
